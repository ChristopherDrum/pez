...
static void calculateStatusBar()
{
    ...
    char score_move_str[24];
    if (score_game) {
        snprintf(score_move_str, 24, "Score:%-3d  Moves:%-4u\0", (int) scoreval, (int) movesval);
    } else {
        snprintf(score_move_str, 24, "Time: %2u:%02u %s\0", (unsigned int) (scoreval % 12) + 1, (unsigned int) movesval, (scoreval < 12) ? "am" : "pm");
    }
    printf("\33[s");                 // save cursor position
    printf("\33[H%s%s", inverse, spaces);
    printf("\33[H %s", objstr);
    printf("\33[1;%dH%s", (TERMINAL_WIDTH - 24), score_move_str);
...
static void initStory(const char *fname, uint8 *story, const uint32 storylen)
{
    ...
    GState->status_bar_len = TERMINAL_WIDTH;
...
static void old_writestr_stdio(const char *str, const uintptr slen)
{
...
static void writestr_stdio(const char *_str, const uintptr slen)
{
    ...
    if (current_len + slen < TERMINAL_WIDTH) {
        ...
        while (token)
        {
            ...
            fwrite(token, 1, token_len, stdout);
            fwrite(" ", 1, 1, stdout);
...
int main(int argc, char **argv)
{
    ...
    const char *fname = (argc >= 2) ? argv[1] : "zork1-r88.z3";
    ...
    printf("\33[2;30r");    // set scroll window to lines 2 - 24
    printf("\33[?25h");     // show cursor
    printf("\33[?12l");     // reset cursor blinking mode
    printf("\33[30;1H");     // set cursor to bottom left
    loadStory(fname);
    while (!GState->quit)
        runInstruction();
    ...
    // printf("\33[c");        //reset terminal to initial state
    return 0;
} // main
