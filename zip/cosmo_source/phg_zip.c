...
char *curword,			/* ascii input word pointer */
     ...
     *esibrks,			/* end of self-inserting break chars */
     ...
     irbrks[] = {SPACE,TAB,CR,FF,PERIOD,COMMA,QMARK,0},
...
void vdump(short blknum);
int getnum(int radix);

void setup();
void statusln();
void flushwrds();
short getlin(short buffer);
void md_putc(char byte);
void zword(char *ptr);
int char_cs(char c);
void makezwrd();
ZIPINT gprpoff(ZOBJECT obj);
char *objloc(ZOBJECT obj);
ZIPINT nxtprp(ZIPINT prpptr);
ZIPINT rotate(ZIPINT word, ZIPINT cnt);
ZIPINT getarg(char var);
short getvar(ZOBJECT var);
void putvar(ZOBJECT var, short value);
void putval(short value);
void bytval(ZOBJECT value);
void ppred(ZIPINT truth);
char *bspltb(ZIPINT byteptr);
char *bsplit(ZIPINT wrdptr);
ZOBJECT getbyt(char **ptr);
ZIPINT getwrd(char **ptr);
ZOBJECT nxtbyt();
ZIPINT nxtwrd();
void newzpc();
void fixzpc();
void fatal(char *message);
char *putstr(char **strptr);
ZIPINT getzchr(ZIPINT zchr, ZIPINT charset);
void putchr(ZIPINT letter);
int dumpfix(ZIPINT letter, int caller);
void p_putchr(ZIPINT letter);
void newlin();
void dumpbuf();
void dmp_pbuf();
void mprnt(char *buf);
...
void zread();
void getasc();
char *nxtasc(char *wordend);
int notbrkc(char c);

char *getpag(char *ptr, char *page);
int blockn(char *page);
int nxtblk(char *ptr, char *page);
char *makeptr(short blk, short byt);
void zip_unlink(short block);
void relink(short block);
void getpre(short block, ZIPINT count);
void getblk(short block, char *loc);
int wrtbyts(ZIPINT *loc, int numbyts);
int rdbyts(ZIPINT *loc, int numbyts);

void zret(ZIPINT rtval);
int sav_res(int fcn);
ZIPINT flagptr(ZOBJECT obj, ZIPINT *flagnum);
...
{   /*  Sysini opens the data file, saves away the name as the default save
    ...
    if ((dataspace = malloc(memreq * 1024)) == 0) THEN {
...
{ /*	NXTINS is the heart of ZIP.  It picks up the next instruction
   ...
   */
	 ...
	 *objptr, *objptr2;
    short ts1;		/* temp for signed operations */
    ZIPINT temp, t1, t2;
...
void zremove(ZOBJECT obj) 		/* OPREMO separated for use by OPMOVE */
...
{  /*	Save and restore routines.  This routine receives the OP as a
  ...
  */
	...
	if (fcn == OPSAVE) THEN		/* r/w impure code accordingly */
	  errcode = wrtbyts((ZIPINT *)dataspace, GTAWRD(PPURBT));
	else
	  errcode = rdbyts((ZIPINT *)dataspace, GTAWRD(PPURBT));
...
ZIPINT flagptr(ZOBJECT obj, ZIPINT *flagnum)	/* return a pointer to a flagword in obj */
...
ZIPINT gprpoff(ZOBJECT obj)	/* Get property pointer for obj objnum */
...
void ppred(ZIPINT truth)		/* do a predicate jump */
{  /*  	Ppred performs a predicate jump based on truth and immediate values.
   ...
   */
    ZOBJECT jump1, jump2; 	/* predicate jump values */
    short offset;
...
{  /*	Makeptr is the heart of the paging scheme.  It manages a doubly-linked
   ...
   */
	...
	if (vpagemap[blk] != mru) THEN {/* in core, but not mru */
	  zip_unlink(blk);			/* unsplice soon to be mru */
...
{  /*	Putchr takes an ASCII character and queues it for output.  If it
   ...
   */
	...
	if ((tailptr == outbuf) && (brkflg != 1)) THEN
	  ...
	  *tailptr = 0;		/* make space into end of line */
...
void p_putchr(ZIPINT letter)		/* put char to pipe (usually printer) */
...
{  /* 	Dmp_pbuf flushes the existing output buffer for the pipe to the
	pipe.
   */
    char *bufptr = p_outbuf;			/* get another pointer */
...
{  /*	Mprnt prints a string assuming that Z_EOL indicates end of line
   ...
   */
    ...
    if (*buf == 0) THEN
      mcrlf();				/* windowed scroll on ending 0 */
    return;
}
