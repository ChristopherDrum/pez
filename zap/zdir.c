...
/* Evaluator routine to call */
/* Routine seems to be either evalbyte() or evalword() */
...
int zd_funct( int dirnum )
{
	int		i;
		int		sts;
		int		argC;		/* Number of function args */
...
/* Whether to output string length */
...
int zd_vocbeg( int dirnum )
{
    ...
    if ( ( sts = evalconst( (ZNUM*)&Vocreclen ) ) != SCOK )
	return( sts );
    ...
    if ( ( sts = evalconst( (ZNUM*)&Vockeylen ) ) != SCOK )
	return( sts );
...
int zd_options(int dirnum)
{
  ...
  sts = evalconst((ZNUM*)&do_script);
  if ((sts != SCOK) && (sts != SCNOTDONE))
    return(zexpected(E_PASS1, "number"));
  sts = ascomma();
  if ((sts != SCOK) && (sts != SCNOTDONE))
    return(sts);
  sts = evalconst((ZNUM*)&do_ask);
  if ((sts != SCOK) && (sts != SCNOTDONE))
...
int zd_lang( int dirnum )
{
  ...
  if ((sts = evalconst((ZNUM*)&lang_id)) != SCOK)
    return(sts);
  ascomma();
  if (!anyarg())
...
int zd_time( int dirnum ) { return zd_nyi( dirnum ); }
