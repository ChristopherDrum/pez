...
void add_marker(SEGMENT *seg, long pc, int first)
{
...
segment). */
void addadj(char *seg1, char *seg2)
{
  int segnum1;
  int segnum2;
...
void addcall(LGSYMBOL *fun, LGSYMBOL *call)
{
...
void addnode(SEGNODE **segP, SEGMENT *data1, SEGMENT *data2)
{
  SEGNODE *newnode;
...
void additem(ITEM **itemlistP, LGSYMBOL *data)
{
  ITEM *newnode;
...
void putlist(ITEM *list) /*I removed *startstr because it isn't passed and it isn't used*/
{
  ITEM *tempitem;
...
void segputl(char *thing_ptr, int thing_len)
{	
  	 int i;
...
void segfile(char *game_name)
{
  ...
  segputl((char *)&funct_page,sizeof(UWORD));
  segputl((char *)&top_page,sizeof(UWORD));
...
void addpages(UWORD start, UWORD end, char table[])
{
  int i;
...
/* This goes through a function's call tree, setting the bit table as it goes
   along */
void funpages(ITEM *funs, SEGMENT *seg)
{
  	 ITEM *temp;
