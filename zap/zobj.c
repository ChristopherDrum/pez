...
int objopen( char *filename ) /* Name of file to open */
{
    if ( ( Objfd = open( filename, O_WRONLY|O_CREAT|O_TRUNC, 0666 ) ) < 0 ) {
	my_fprintf( stderr, "Can not open object file \"%s\"", filename, NULL, NULL, NULL);
	return( SCNOFILE );
    }
...
int objclose( int wrtF ) /* Whether to keep */
{
	int		i;
	int		sts;
...
int objput( UBYTE *dataP, int datalen )
/* Data address and Number of bytes of data */
{
int		sts;
    if (build_freq) return(SCOK);
    while( datalen-- > 0 )
...
int objputw( UWORD w ) /* Word to output */
{
...
int objseek( long newpos, int truncF )
{
...
int objvoc( VNODE *nodeP ) /* Addr of vocab node */
{
VLABEL		*vlP;		/* Label block data */
LGSYMBOL	*symP;
FIXUP *vf;
    if (build_freq) return(SCOK);
...
static int insvnode( VNODE *nodeP )
{
	int		keylen;		/* Length of comparison key */
	VNODE		**linkPP;	/* Ptr to insertion link */
	VNODE		*nnodeP;	/* Ptr to next node */
    if (build_freq) return(SCOK);
...
int voclabel( LGSYMBOL *symP )/* Ptr to the symbol table entry */
{
	int		sts;
	VLABEL		*vlP;		/* Label block */
...
int reccmp(unsigned char *r1, unsigned char *r2, int len)
{
	for (;*r1 == *r2; r1++, r2++)
		if (--len == 0)
			return(0);
