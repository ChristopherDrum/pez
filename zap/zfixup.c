...
static int fix_syms( FIXUP **fix_list )
{
  	 int sts;
  	 FIXUP **fixP;
  	 FIXUP *fix;
...
static int fixupone(FIXUP *fix) /* The fixup block */
{
  	   LGSYMBOL        *sym;
  		int		sts;
  		UWORD		value;
...
static int res_fix( FIXUP **fix_list ) /* Addr of fixup list */
{
		FIXUP		*fixP;		/* Fixup block ptr */
		FIXUP		**fixPP;	/* Fixup block list link addr */
...
int makefixup(LGSYMBOL *symP, int class, long addr, ZNUM off)
{
  		FIXUP		*fixP;
...
static int undefsym( LGSYMBOL *symP )
{
