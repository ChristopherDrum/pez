...
int tklex( LEXBUF **lxPP, int *lxXP, char *filename )
	...
	// char		*filename;	/* Name of file to scan */
{
	...
	switch( tktype ) {		/* Process by token type */
		...
		sts = lexadd( &lxP, &lxX, tktype, (char*)&val, sizeof(ZNUM) );
		break;
...
int lexdirop(LEXBUF **lxPP, int *lxXP, char *tokenP, FILE *fP, char *filename)
...
// char		*filename;	/* Current filename */
{
	...
	int		sts;
	int		flags;
	ODSYMBOL	*odsymP;	/* Symtab entry ptr */	
	char		*token;
	char		tch;
	char		*ttok;
	token = get_buffer();
  ...
  if ( ( flags & ST_DIRECTIVE ) == 0 ) {
    ...
    free_buffer();
    return( lexadd( lxPP, lxXP, TKOPCODE, (char*)&odnum, sizeof(UWORD) ) ); }
...
int lexlinemark( LEXBUF **lxPP, int *lxXP, UWORD linenum, FILE *fP )
	// LEXBUF		**lxPP;		/* Ptr to lexbuf ptr var */
	// int		*lxXP;		/* lexbuf index */
...
int gettoken( FILE *fP, char **token_start, int bolF )
// FILE		*fP;		/* Ptr to file block */
// char		**token_start;	/* Where to put token string */
// int		bolF;		/* If beginning of line */
{
	char	numberF;	/* is a number? */
	int		c;		/* Character */
	int		tklen;		/* Lenth of token */
	char	*tokenP;
	int		token_diff;
	int     buf_siz;
...
int lexadd( LEXBUF **lxPP, int *lxXP, int tktype, char *tokenP, int tokenL )
