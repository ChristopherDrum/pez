...
void fw_insert(fw_node thing, int at, int kill);
...
int addfstr( char *strP )/* Addr of zstring */
{
...
UBYTE *translate_char(UBYTE *strP, UBYTE *chrp)
{
...
UBYTE *zstr( UBYTE *strP, int freqF, int reqlen )
	// 	UBYTE		*strP;		/* Ptr to input string */
	...
	// int		reqlen;		/* Required length, if any */
{
	...
	struct freq_node *fnode;		/* Frequent string node */
...
int zstrlen( UBYTE *strP )/* Ptr to string */
{
		int		len;		/* Length of string */
...
void zcset( UBYTE *csP )/* New character set */
{
		int		i;		/* Scratch */
		int		ch;		/* Char val */
...
void startzstr( UBYTE *strP, int reqlen )
...
void addzchar( UBYTE ch )/* byte value to add */
{
...
int zchar_size(UBYTE ch, int *cset_ptr)
{
  int val, cset;
...
int fw_get_hash(UBYTE *str, int len)
{
  int hash = 0;
...
fw_node fw_lookup(UBYTE *str, int len, int hash)
{
  fw_node buck;
...
fw_node fw_add(UBYTE *str, int len, int zlen, fw_node parent, int hash)
{
  fw_node buck;
  UBYTE *nstr;
  fw_node_ct++;
...
void prune_parents(fw_node thing)
{
...
fw_node any_frequent_words(fw_node thing)
{
  for (; thing; thing = thing->fw_parent)
    if (thing->fw_id > 0) return(thing);
  return(0); }

void dump_frequent_words(char *game_name)
{
  int i, j;
  fw_node buck;
  int fw_allocated = 0;
...
void print_stuff(char *game_name, int cnt)
{
  int i, j;
  fw_node temp;
  FILE *fw_file;
...
int check_name(fw_node thing, char *str)
{
...
void update_parents(fw_node thing, int ct)
{
...
void update_kids(fw_node thing, int fudge)
{
