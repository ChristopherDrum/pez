...
STABLE *symnew(int hsize, int flags)
{
    STABLE *table;
    SYMBOL **p;
    int n;
...
void symempty(STABLE *table, int size)
{
		SYMBOL *this;
		SYMBOL *next;
...
SYMBOL *symenter(STABLE *table, char *name, int size)
...
SYMBOL *symlookup(STABLE *table, char *name)
//   STABLE *table;		/* the symbol table */
// 		char *name;		/* the symbol name */
{
		SYMBOL *sym;
...
int symwalk(STABLE *table, int (*rtc)())
//   STABLE *table;	/* the symbol table */
