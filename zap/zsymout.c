...
void init_sym_stuff(char *fname)
{
	int i;
	unsigned short ver = ST_VERSION;
	if (!build_freq) {
	  sym_open(fname);
	  st_output_thing((char *)&ver,2);
...
void finish_up_syms(char *filename)
{
...
ST_SYMTAB *sym_dump(STABLE *syms, long stoff, int is_global)
{
  	 int i;
  	 int j;
  int process_symbol();
...
int process_symbol(SYMBOL *symbl)
{
  	 LGSYMBOL *lg_symbl;
  ...
  if (lg_symbl->lg_space > SPACESUSED - 1) {
    my_fprintf(stderr,"ZAP: Bad space %d.\n",
	       lg_symbl->lg_space, NULL, NULL, NULL);
    my_exit(SCERR);
  }
...
void process_add_info(LGSYMBOL *sym)
{
	...
	STR_FIXUP_NODE *new_struct;
		...
		case VARIABLE_SPACE:
			...
			st_output_thing((char *)&type_info,1);
			break;
		case FUNCTION_SPACE:
			st_output_thing((char *)&(sym->lg_add_info.function.min_args),1);
			st_output_thing((char *)&(sym->lg_add_info.function.max_args),1);
			st_output_thing((char *)&(sym->lg_add_info.function.value_space),1);
			st_output_skip(1);
			st_output_thing((char *)sym->lg_add_info.function.syms,
							sizeof(ST_SYMTAB));
			break;
		case TABLE_SPACE:
			temp_tbl = sym->lg_add_info.table;
			while (temp_tbl->type == ARRAY_TYPE) {
			  st_output_thing((char *)&(temp_tbl->type),1);
			  st_output_thing((char *)&(temp_tbl->it.array.el_size),
					  sizeof(short));
			  st_output_thing((char *)&(temp_tbl->it.array.num_els),
					  sizeof(short));
...
SF_PTR process_sym_name(char *name)
{
...
void make_new_links(ST_D_LNK **n_link_p, ST_D_LNK **s_link_p, char *d_name, long d_value, SF_PTR d_loc)
{
  ST_D_LNK *get_link();
...
ST_D_LNK **to_array(ST_D_LNK *d_list, unsigned int d_num)
{
	static ST_D_LNK **arr;
...
ST_D_LNK **sort_by_name(ST_D_LNK *d_list,unsigned int a_num)
{
  	 int gap;
  	 int i;
  	 int j;
...
ST_D_LNK **sort_by_val(ST_D_LNK *d_list, unsigned int a_num)
{
	int gap;
	int i;
	int j;
...
int sym_open(char *filename) /* Name of symbol file. Should be "game.SYMS" */
{
...
void st_output_compat_info(char *filename)
{
	int fd;
...
void do_sym_fix(ST_SYMTAB *head, SF_PTR loc)
{
	SF_PTR save_loc;
...
void st_output_thing(char *thing_ptr, int thing_len)
{	
	int i;
  if (build_freq) return;
...
int st_output_arr(ST_D_LNK **dec_arr, unsigned int arr_len, int unique)
{
  	 unsigned int i;
  SF_PTR temp;
  int ct = 0;
