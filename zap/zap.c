...
static int usage( char *msgP, char *tokenP )
{
...
void odinit()
{
    int		odnum;		/* O/d number */
    ODDISP		*odP;		/* Ptr to a table */
    ODSYMBOL	*odsymP;	/* Ptr to a symbol */
...
void def_sym( STABLE *symtabP, char *nameP, UWORD flags, int value, unsigned char space )
...
void write_character_counts()
{
  ...
  for (i = 0; i < 256; i++) {
    ...
    if (chrs[i] == 0)
      my_printf ("  ^@", NULL, NULL, NULL, NULL);
    else if (chrs[i] < ' ')
      my_printf("  ^%c", chrs[i] + 'A', NULL, NULL, NULL);
    else if (chrs[i] == ' ')
      my_printf("  SP", NULL, NULL, NULL, NULL);
    else if (chrs[i] == 127)
      my_printf("  ^?", NULL, NULL, NULL, NULL);
    else if (chrs[i] > 127)
      my_printf(" %3o", chrs[i], NULL, NULL, NULL);
    else
      my_printf("   %c", chrs[i], NULL, NULL, NULL);
    my_printf("%11d\n", character_counts[i], NULL, NULL, NULL);
  }
}
...
void print_cpu_time(int cpu_time)
{
  int any = 0;
  int h, m, s;
  ...
  s = cpu_time / sec_fact;
  my_printf("Used ", NULL, NULL, NULL, NULL);
  if (h > 0) {
    my_printf("%d:", h, NULL, NULL, NULL);
    any = 1; }
  if (any)
    my_printf("%02d:", m, NULL, NULL, NULL);
  else if (m > 0) {
    my_printf("%d:", m, NULL, NULL, NULL);
    any = 1; }
  if (any)
    my_printf("%02d\n", s, NULL, NULL, NULL);
  else
...
void print_elapsed_time(int starting_time)
{
  ...
  m = elapsed / 60;
  s = elapsed % 60;
  my_printf("Elapsed time ", NULL, NULL, NULL, NULL);
  if (h > 0) {
    my_printf("%d:", h, NULL, NULL, NULL);
    any = 1; }
  if (any)
