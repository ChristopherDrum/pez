...
void res_map(long type, long length, long pointer, int which)
{
...
void res_long(long num, int where)
{
...
void res_short(short num, int where)
{
  res_buf[where++] = (num >> 8) & 0377;
  res_buf[where] = num & 0377;
}

void res_string(char *str, int where)
{
  strcpy(&res_buf[where], str);
}

void write_a_res(char *fname, char *rt, int ready, int real)
{
  int fd;
  int lenloc;
  char new_name[1024];
...
void stuff_short(unsigned char *buf, int offs, int what)
{
  short *foo = (short *)buf;
  foo[offs / 2] = what;
}

res_type *add_a_resource_type(char *rtype)
{
...
void add_a_resource(char *rtype, int id, int data_len, unsigned char *data)
{
